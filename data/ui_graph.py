...
class Interaction(Data, Graph):
    ...
    def __create_sparse_bipartite_adjacency(self, self_connection=False):
        ...
        row_idx = [self.user[pair[0]] for pair in self.training_data]  # user2id
        col_idx = [self.item[pair[1]] for pair in self.training_data]  # item2id
        user_np = np.array(row_idx)
        item_np = np.array(col_idx)
        ratings = np.ones_like(user_np, dtype=np.float32)
        # create compress sparse row matrix
        tmp_adj = sp.csr_matrix((ratings, (user_np, item_np + self.user_num)), shape=(n_nodes, n_nodes),
                                dtype=np.float32)
        adj_mat = tmp_adj + tmp_adj.T
        if self_connection:
            adj_mat += sp.eye(n_nodes)  # add identity matrix
        return adj_mat
    ...
    def convert_to_laplacian_mat(self, adj_mat):
        ...
        tmp_adj = sp.csr_matrix((ratings_keep, (user_np_keep, item_np_keep + adj_shape[0])),
